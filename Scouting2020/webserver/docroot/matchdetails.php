<?php
  ...
  //
  // Competition System - Show match details
  //
    ...
    // used to pass back to referring matchteameval page
    if (isset($_GET["matchteamnum"])) $matchteamnum=$_GET["matchteamnum"]; else $matchteamnum = NULL;
...
// put back in if edit needed -- JLV
	if ($edit)
	{
		...
		print "<form method=\"POST\" action=\"/matchdetails.php?edit=3&final={$final}"
		    . "&type={$matchidentifiers["type"]}&matchnum={$matchidentifiers["matchnum"]}\">\n";
	}

    $editURL = "/matchdetails.php?&final={$final}&type={$matchidentifiers["type"]}&matchnum={$matchidentifiers["matchnum"]}";
...
*/
    ...
    // if called by matchteameval, leave link to return
    if ($matchteamnum)
      print "<a href=\"/matchteameval.php?teamnum={$matchteamnum}&final={$final}&type={$matchidentifiers["type"]}&matchnum={$matchidentifiers["matchnum"]}\">Back to Match Team</a>\n &nbsp;&nbsp;&nbsp;\n";
    ...
    if (($time_array['can_set']) && (! ($edit)))
      print "<form method=\"POST\" action=\"/matchdetails.php?/matchdetails.php?&final={$final}&type={$matchidentifiers["type"]}&matchnum={$matchidentifiers["matchnum"]}\">\n";
