...
rem
rem Dumps customizations so they can be loaded on other databases
rem
...
set tables=custom_param

set today=%date:~10,4%-%date:~4,2%-%date:~7,2%

mysqldump %options% -u %dbuser% --password=%dbpass% %dbname% %tables% > ..\dumps\dump-customizations-%today%.dmp
